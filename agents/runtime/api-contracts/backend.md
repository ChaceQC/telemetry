...
- 当前边界：只读已保存 dashboard `config.panels`；不支持未保存草稿 config，不写 dashboard，不接 ClickHouse，不做真实图表渲染，不做变量替换、模板、缓存、后台任务或告警。
