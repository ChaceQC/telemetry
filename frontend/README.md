...
- `/metrics`：调用 `GET /api/v1/query/metrics`，支持项目 ID、指标名、来源、时间范围、数量和 `cursor` 筛选。
...
未登录或会话恢复中时，查询页显示登录提示并暂停请求；登录后可刷新或提交筛选条件重新查询。当前页面先提供筛选表单、刷新状态、错误/空态、结果列表和最小分页；图表、日志上下文、事件时间线细节和指标聚合窗口后续拆分。
