...
未登录或会话恢复中时，查询页显示登录提示并暂停请求；登录后可刷新或提交筛选条件重新查询。当前页面提供筛选表单、刷新状态、错误/空态、结果列表、最小分页，`/metrics` 当前页指标趋势图，以及 `/logs` 单条日志上下文面板；当前页混合多个指标名或单位时会显示趋势不可用提示。事件时间线细节、指标聚合窗口、多序列对比和跨页趋势后续拆分。
