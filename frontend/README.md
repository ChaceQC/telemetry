...
- `/metrics`：调用 `GET /api/v1/query/metrics`，支持项目 ID、指标名、来源、时间范围、数量和 `cursor` 筛选，并在当前页结果上展示 value 随 received_at 变化的轻量趋势图。
- `/logs`：调用 `GET /api/v1/query/logs`，支持项目 ID、日志级别、来源、时间范围、数量和 `cursor` 筛选。
...
未登录或会话恢复中时，查询页显示登录提示并暂停请求；登录后可刷新或提交筛选条件重新查询。当前页面提供筛选表单、刷新状态、错误/空态、结果列表、最小分页，以及 `/metrics` 当前页指标趋势图；日志上下文、事件时间线细节、指标聚合窗口、多序列对比和跨页趋势后续拆分。
