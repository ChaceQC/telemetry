...
当前阶段提供项目范围 dashboard CRUD 后端基础、全局 `time_range` 最小保存校验、顶层 `variables` 最小保存校验和已保存 panel 的只读查询预览 API；panel preview 会在 panel query 未显式设置对应时间边界时继承 dashboard 全局时间范围。不包含前端页面、真实图表渲染、变量替换、模板、自动刷新、ClickHouse 查询或告警规则。所有接口均需要 `Authorization: Bearer <access_token>`，且 token 对应用户必须启用。普通用户只能访问自己有项目角色的 dashboard；超级用户可访问全部已存在项目。
