...
当前阶段提供项目范围 dashboard CRUD 后端基础、全局 `time_range` 最小保存校验和已保存 panel 的只读查询预览 API；panel preview 会在 panel query 未显式设置对应时间边界时继承 dashboard 全局时间范围。不包含前端页面、真实图表渲染、变量、模板、自动刷新、ClickHouse 查询或告警规则。所有接口均需要 `Authorization: Bearer <access_token>`，且 token 对应用户必须启用。普通用户只能访问自己有项目角色的 dashboard；超级用户可访问全部已存在项目。
...
列表响应为对象 envelope：`{"items": [...], "limit": 50, "offset": 0, "total": 1}`。字段规则：`name` 为 1 到 100 字符，`description` 最多 500 字符；`layout` 和 `config` 必须是 JSON 对象或数组，创建时默认 `{}`，单字段序列化后不超过 64 KiB，嵌套深度不超过 32，复杂度不超过 4096 个节点，且不能包含 `NaN`、`Infinity` 或 `-Infinity`。当 `config` 是对象且包含 `time_range` 时，`time_range` 必须是对象；相对范围保存为 `{"mode":"relative","relative":"15m|1h|6h|24h|7d"}`，`mode/relative` 会裁剪首尾空白并按裁剪后值保存；绝对范围保存为 `{"mode":"absolute","from":"ISO 8601","to":"ISO 8601"}`，`mode/from/to` 会裁剪首尾空白并保留裁剪后的原始字符串，`from/to` 必须可解析并可比较为 ISO 8601 时间且 `from < to`。当 `config` 是对象且包含 `panels` 时，`panels` 必须是数组；每个 panel 必须是对象，包含 `id`（1 到 64 字符）、`title`（1 到 120 字符）、`type`（`metrics`、`logs`、`events`、`traces`、`topology` 之一）和对象类型的 `query`；`id/title/type` 会先裁剪首尾空白再校验和保存，同一 `panels` 内重复 `id` 也按裁剪后值判断；可选 `layout` 必须是对象，包含非负 `x/y` 和正数 `w/h`。旧版 `{"refresh_seconds": 30}`、空 `{}`、没有 `time_range` 或没有顶层 `panels` 的 JSON 结构仍保持兼容。更新至少提供一个字段；未传 `layout/config` 时保持原值，传入空对象/空数组有效，`description=null` 表示清空描述，`name/layout/config=null` 返回 `422`。错误边界：缺少或无效 token 返回 `401`；项目不存在、无项目成员关系或 dashboard 不在指定项目下返回 `404`；角色不足返回 `403`；数据库完整性冲突返回 `409`；字段、路径参数、分页参数、非法 `config.time_range` 或非法 panel config 返回 `422`。

已保存 panel 查询预览：已认证用户可通过 `GET /api/v1/projects/{project_id}/dashboards/{dashboard_id}/panels/{panel_id}/preview` 对单个已保存 panel 生成只读预览。权限要求与读取 dashboard 一致，目标项目至少 `viewer`；项目不存在、普通用户无项目成员关系、dashboard 不属于该项目、不存在或 `panel_id` 未命中已保存 `config.panels[].id` 时返回 `404`。响应包含 `project_id`、`dashboard_id`、`panel_id`、`title`、`panel_type`、原始 `query` 和 `preview`。`metrics` panel 当前使用关系库 `GET /api/v1/query/metrics/aggregate` 同源能力返回 `{"kind":"metrics","mode":"aggregate","items":[...]}`；`logs`、`events`、`traces` 返回最近样本 `items`；`topology` 返回 `nodes` 和 `edges` 摘要。`query` 只读取当前查询 API 已支持的白名单字段：通用 `source`、`occurred_from`、`occurred_to`、`limit`，以及各类型已有的 `name`、`level`、`keyword`、`trace_id`、`span_id`、`request_id`、`user_id`、`status_code`、`duration_min_ms`、`duration_max_ms`、`window`、`aggregation`、`type/event_type` 等；未知字段忽略。若 dashboard 保存了 `config.time_range`，preview 会在 panel query 未显式设置对应时间边界时转换为 query service 的 `occurred_from` / `occurred_to`；`relative` 支持 `15m/1h/6h/24h/7d` 并基于服务端当前 UTC 时间生成查询范围，`absolute` 使用保存的 `from/to`。panel 显式 `occurred_from` / `occurred_to` 分别优先，支持单边覆盖。`limit` 范围为 `1..100`，非法 query 字段类型、非法时间、非法聚合窗口或 duration 下界大于上界等沿现有 query service 语义返回 `422`。该接口只读取已保存 dashboard，不支持未保存草稿 config，不写入 dashboard，不接 ClickHouse，不执行图表渲染、变量替换、模板或告警逻辑。
