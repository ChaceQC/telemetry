...
到期扫描接口 `POST /api/v1/alerts/evaluations/run-due` 无请求体，当前也不实现 `limit` 或 `project_id` 可选参数。服务端读取 enabled 告警规则，按状态表 `next_evaluate_at` 判断是否 due；缺少状态、状态缺少 `next_evaluate_at`，或 `next_evaluate_at <= checked_at` 时视为 due。due 规则复用手动评估的 metrics 阈值语义执行，成功后写入 `status=firing/ok/no_data`、`last_evaluated_at`、`next_evaluate_at=checked_at+evaluation.interval_seconds`、`last_result` 和清空 `last_error`；当前非 metrics 或不满足 API-0026 执行语义的 enabled 规则写入 `status=error` 和 `last_error`，并推进下一次评估时间。未到期规则只计入 skipped，不更新状态。响应为本轮扫描摘要：`checked_at`、`evaluated_count`、`skipped_count`、`created_state_count`、`updated_state_count`、`items[]`，每项包含 `rule_id`、`project_id`、`old_status`、`new_status`、`due`、`next_evaluate_at` 和 `error_summary`。
