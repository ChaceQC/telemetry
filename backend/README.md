...
当前阶段提供项目范围 dashboard CRUD 后端基础、全局 `time_range` 最小保存校验、顶层 `variables` 最小保存校验和已保存 panel 的只读查询预览 API；panel preview 会在 panel query 未显式设置对应时间边界时继承 dashboard 全局时间范围，并在执行前用已保存变量 default 替换顶层 query 字段中的完整 `${变量名}` 模板。不包含前端页面、真实图表渲染、请求时变量覆盖、模板 dashboard、自动刷新、ClickHouse 查询或告警规则。所有接口均需要 `Authorization: Bearer <access_token>`，且 token 对应用户必须启用。普通用户只能访问自己有项目角色的 dashboard；超级用户可访问全部已存在项目。
...
已保存 panel 查询预览：已认证用户可通过 `GET /api/v1/projects/{project_id}/dashboards/{dashboard_id}/panels/{panel_id}/preview` 对单个已保存 panel 生成只读预览。权限要求与读取 dashboard 一致，目标项目至少 `viewer`；项目不存在、普通用户无项目成员关系、dashboard 不属于该项目、不存在或 `panel_id` 未命中已保存 `config.panels[].id` 时返回 `404`。响应包含 `project_id`、`dashboard_id`、`panel_id`、`title`、`panel_type`、原始 `query` 和 `preview`。`metrics` panel 当前使用关系库 `GET /api/v1/query/metrics/aggregate` 同源能力返回 `{"kind":"metrics","mode":"aggregate","items":[...]}`；`logs`、`events`、`traces` 返回最近样本 `items`；`topology` 返回 `nodes` 和 `edges` 摘要。`query` 只读取当前查询 API 已支持的白名单字段：通用 `source`、`occurred_from`、`occurred_to`、`limit`，以及各类型已有的 `name`、`level`、`keyword`、`trace_id`、`span_id`、`request_id`、`user_id`、`status_code`、`duration_min_ms`、`duration_max_ms`、`window`、`aggregation`、`type/event_type` 等；未知字段忽略。执行前会读取已保存 `config.variables`，仅当已保存 panel `query` 顶层字段值完整匹配 `${变量名}` 时替换为该变量 `default`，再进入现有 query 白名单校验和 preview 执行；`text/select` default 保持字符串，`number` default 保持数字。未知变量、变量无 `default`、模板语法非法或替换后类型不满足现有 query 校验均返回 `422`；不支持请求时变量覆盖、部分字符串拼接替换或数组/对象深层模板替换。若 dashboard 保存了 `config.time_range`，preview 会在变量替换后、panel query 未显式设置对应时间边界时转换为 query service 的 `occurred_from` / `occurred_to`；`relative` 支持 `15m/1h/6h/24h/7d` 并基于服务端当前 UTC 时间生成查询范围，`absolute` 使用保存的 `from/to`。panel 显式 `occurred_from` / `occurred_to` 分别优先，支持单边覆盖。`limit` 范围为 `1..100`，非法 query 字段类型、非法时间、非法聚合窗口或 duration 下界大于上界等沿现有 query service 语义返回 `422`。该接口只读取已保存 dashboard，不支持未保存草稿 config，不写入 dashboard，不接 ClickHouse，不执行图表渲染、模板 dashboard、自动刷新或告警逻辑。
