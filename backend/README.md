...
当前阶段提供项目范围 dashboard CRUD 后端基础和已保存 panel 的只读查询预览 API，不包含前端页面、真实图表渲染、变量/时间范围高级配置、ClickHouse 查询或告警规则。所有接口均需要 `Authorization: Bearer <access_token>`，且 token 对应用户必须启用。普通用户只能访问自己有项目角色的 dashboard；超级用户可访问全部已存在项目。
...
已保存 panel 查询预览：已认证用户可通过 `GET /api/v1/projects/{project_id}/dashboards/{dashboard_id}/panels/{panel_id}/preview` 对单个已保存 panel 生成只读预览。权限要求与读取 dashboard 一致，目标项目至少 `viewer`；项目不存在、普通用户无项目成员关系、dashboard 不属于该项目、不存在或 `panel_id` 未命中已保存 `config.panels[].id` 时返回 `404`。响应包含 `project_id`、`dashboard_id`、`panel_id`、`title`、`panel_type`、原始 `query` 和 `preview`。`metrics` panel 当前使用关系库 `GET /api/v1/query/metrics/aggregate` 同源能力返回 `{"kind":"metrics","mode":"aggregate","items":[...]}`；`logs`、`events`、`traces` 返回最近样本 `items`；`topology` 返回 `nodes` 和 `edges` 摘要。`query` 只读取当前查询 API 已支持的白名单字段：通用 `source`、`occurred_from`、`occurred_to`、`limit`，以及各类型已有的 `name`、`level`、`keyword`、`trace_id`、`span_id`、`request_id`、`user_id`、`status_code`、`duration_min_ms`、`duration_max_ms`、`window`、`aggregation`、`type/event_type` 等；未知字段忽略。`limit` 范围为 `1..100`，非法 query 字段类型、非法时间、非法聚合窗口或 duration 下界大于上界等沿现有 query service 语义返回 `422`。该接口只读取已保存 dashboard，不支持未保存草稿 config，不写入 dashboard，不接 ClickHouse，不执行图表渲染、变量替换、模板或告警逻辑。
