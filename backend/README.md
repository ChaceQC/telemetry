...
Dashboard JSON 导出使用 `GET /api/v1/projects/{project_id}/dashboards/{dashboard_id}/export`，权限与读取 dashboard 一致，目标项目至少 `viewer`。响应是可移植文档，只包含 `schema`、`version`、`name`、`description`、`layout`、`config`；当前 `schema` 固定为 `telemetry.dashboard`，`version` 固定为 `1`，不会包含数据库 `id`、`project_id`、创建/更新用户或创建/更新时间。导入使用 `POST /api/v1/projects/{project_id}/dashboards/import`，请求体为 `{"document": <导出文档>, "name": "可选覆盖", "description": "可选覆盖或 null"}`；未提供覆盖时使用导出文档内的 `name/description`。导入会在目标项目下创建普通 dashboard，权限与创建 dashboard 一致，目标项目至少 `editor`；普通用户无项目成员关系或项目不存在返回 `404 项目不存在`，`viewer` 返回 `403 无项目权限`。导入文档必须包含 `schema/version/name/description/layout/config`，且不得包含 `id/project_id/created_by_user_id/updated_by_user_id/created_at/updated_at` 等实例字段；非法 schema/version、缺必填字段、未声明字段、非法 `layout/config`、超大/过深/过复杂 JSON、非有限数、非法 panel/time_range/variables 均返回 `422`。导入不支持文件上传存储、批量导入、覆盖已有 dashboard、模板市场、分享/只读或跨项目权限提升。
