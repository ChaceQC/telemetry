...
from app.schemas.query import EventQueryResponse, LogQueryResponse, MetricQueryResponse
...
@router.get(
    "/logs",
    response_model=list[LogQueryResponse],
    summary="查询日志",
)
def list_logs(
    query_service: Annotated[QueryService, Depends(get_query_service)],
    ...
    occurred_to: datetime | None = None,
    limit: Annotated[int, Query(gt=0, le=500)] = 100,
) -> list[LogQueryResponse]:
    try:
        logs = query_service.list_logs(
            user=current_user,
            project_id=project_id,
            level=level,
            source=source,
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(error)) from error
    return [LogQueryResponse.model_validate(log) for log in logs]
...
def list_metrics(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(error)) from error
    return [MetricQueryResponse.model_validate(metric) for metric in metrics]
