...
7. 浏览器联调、接口联调或端到端验证完成后，必须关闭本次由自己启动的前端、后端、浏览器、数据库临时库或预览服务，并确认对应 PID、端口和临时资源已释放；不得关闭用户、总 agent 或其他子 agent 启动的进程。
...
10. 部署后用户访问必须支持 `https://域名/xxx` 形式；前端路由、静态资源路径、API base URL、CORS、Trusted Host 和宿主机 Nginx 反代规则必须围绕域名路径访问设计，不得只支持裸 IP、直连端口或根路径。
...
22. 任何部署、联调或前端路由/API 配置变更，都必须检查 `https://域名/xxx` 访问形态是否仍然可用；如依赖子路径部署，必须同步 Vite base、React Router basename、后端 API 前缀和 Nginx rewrite/反代配置。
23. 总 agent 启动子 agent 后不得频繁打扰、催促或轮询；除交付结果、明确阻塞、超时、用户要求或必须追加边界约束外，不主动插话干扰子 agent 工作。
24. 后续启动开发、测试或审计子 agent 时，思考强度默认选择 `xhigh`，并在启动记录中写明任务边界、可否再启动测试 agent、不得代跑完整测试流程和只清理自有资源。
25. 本地开发和测试环境为 Windows 11，命令必须优先使用 PowerShell、Windows 可用命令或跨平台工具；不得把 Linux 专用命令、路径或 shell 语法当作本地可用前提。
26. 前端开发和前端测试涉及真实浏览器时，默认使用 Playwright 操作 Microsoft Edge；如 Edge 不可用，必须记录例外原因和替代浏览器。
27. 本地验证不得启动本机 Docker；需要 MySQL 时直接使用本地 MySQL 服务、临时库或本地 MySQL 实例，并记录连接方式、临时库名和清理结果。
28. 虽然开发和测试在 Windows 11 上进行，代码、脚本、配置、依赖和路径处理仍必须考虑 Debian 部署兼容性，不得引入只在 Windows 可用的生产运行假设。
...
9. `0.2.7`：增加 `/logs` 到 `/traces` 的反向跳转入口，形成 trace/log 基础互跳闭环。
10. `0.2.8`：增加关系库 trace 服务拓扑最小后端查询。
...
17. 总 agent 启动子 agent 后默认等待其完成或进入明确阻塞；除交付、阻塞、超时、用户要求或必须补充边界约束外，不频繁发送追问、状态催促或重复轮询。
18. 后续所有新启动的开发、测试和审计子 agent 默认使用 `xhigh` 思考强度；若因工具限制无法设置，必须在 `AGENT_COMMUNICATION.md` 记录例外原因。
19. 启动任何开发、测试或审计子 agent 时，必须传达 Windows 11 本地命令边界、前端浏览器验证使用 Playwright + Microsoft Edge、本地不启动 Docker 且 MySQL 使用本地服务/临时库、同时保留 Debian 部署兼容性。
